...
class TcpProxyConnector implements TcpServerHandler {
	...
	public void register(Selector selector) {
    	System.out.println("TcpProxyConnector selector.hashCode() = "+selector.hashCode());
		this.selector = selector;

		try {
			clientChannel.configureBlocking(false);

			final InetSocketAddress socketAddress = new InetSocketAddress(config.getRemoteHost(), config.getRemotePort());
			...
			if (LOGGER.isLoggable(Level.WARNING))
				LOGGER.log(Level.WARNING, "Could not connect to " + config.getRemoteHost() + ":" + config.getRemotePort(), exception);
		}
	}
